...
class AlmaBib:
    ...
    @property
    def has_908(self):
        return bool(len(self.record.get_fields("908")))
    ...
    def generate_updated_record(self, new_oclc_number, numbers_from_019=[]):
        ...
        subfield_zs = [
            pymarc.Subfield(code="z", value=f"(OCoLC){value}")
            for value in numbers_from_019
        ]

        # add fixed oclc field
        new_record.add_field(
            pymarc.Field(
                tag="035", indicators=["", ""], subfields=subfield_a + subfield_zs
            ),
        )
        if self.has_908:
            new_record.add_field(
                pymarc.Field(
                    tag="997",
                    indicators=["", ""],
                    subfields=[pymarc.Subfield(code="a", value="SendAuth")],
                ),
            )
        return new_record

    def update_035a(self, new_oclc_number, numbers_from_019=[]):
        new_record = self.generate_updated_record(
            new_oclc_number=new_oclc_number, numbers_from_019=numbers_from_019
        )
        xml = (
            "<bib>" + str(pymarc.marcxml.record_to_xml(new_record).decode()) + "</bib>"
        )
        ...
        resp = requests.put(
            f"https://api-na.hosted.exlibrisgroup.com/almaws/v1/bibs/{self.mms_id}",
            headers=headers,
            params=params,
            data=xml,
        )
