...
def test_generate_update_bib_has_019_in_035a(alma_bib):
    new_record = AlmaBib(alma_bib).generate_updated_record(
        new_oclc_number="123", numbers_from_019=["555", "222"]
    )
    control_numbers = []
    for f in new_record.get_fields("035"):
...
def test_generate_update_bib_does_has_a_997_when_there_is_a_908(alma_bib_with_908):
    new_record = AlmaBib(alma_bib_with_908).generate_updated_record(
        new_oclc_number="123", numbers_from_019=["555", "222"]
    )
    assert (new_record.get("908")) != None
    assert (new_record.get("997")) != None
